	...
	<body>
		...
		<div id="wrapper" class="clearfix">
			...
			<header id="header">
				...
				<span class="logo pull-right">
					<img src="{{request()->root()}}/public/assets/images/_smarty/@lang('site.getContent',['ar' => 'logo-footer.ar.png','en' => 'logo.png'])" alt="admin panel" height="35" />
				</span>
						...
						<li class="dropdown pull-right">
							<a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
								@if(isset(auth()->guard('user')->user()->picture))
									<img class="user-avatar" alt="" src="{{ auth()->guard('user')->user()->getPicture() }}" height="34" /> 
								@else
								...
								<span class="user-name">
									<span class="hidden-xs">
										{{ auth()->guard('user')->user()->name }} <i class="fa fa-angle-down"></i>
                                    </span>
								</span>
							</a>
							<ul class="dropdown-menu hold-on-click">
								...
								<li><!-- logout -->
									<a href="{{ request()->root() }}/logout"><i class="fa fa-power-off"></i> تسجيل الخروج </a>
								</li>
							</ul>
						</li>
						<!-- /USER OPTIONS -->
        ...
        @if(auth()->guard('user')->user()->unreadNotifications->count() > 0)
            <script type="text/javascript">
			    _toastr("لديك عدد {{ auth()->guard('user')->user()->unreadNotifications->count() }} تنبيهات جديدة","top-left","success","{{ request()->root() }}/userAccount/notifications");
            </script>
        @endif
		<script type="text/javascript">
			/* 
